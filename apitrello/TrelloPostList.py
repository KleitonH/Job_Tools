...
# Função para verificar se um card já existe na lista
def card_exists(list_id, card_name):
    ...
    if response.status_code == 200:
        cards = response.json()
        return next((card['id'] for card in cards if card['name'] == card_name), None)
    else:
...
def create_card(list_id, card_name):
    existing_card_id = card_exists(list_id, card_name)
    if existing_card_id:
        print(f"Card '{card_name}' já existe.")
    ...
    if response.status_code == 200:
        print(f"Card '{card_name}' criado com sucesso.")
        return response.json()['id']
    else:
        print(f"Erro ao criar card '{card_name}': {response.text}")
        return None

# Função para verificar se uma checklist já existe em um card
def checklist_exists(card_id, checklist_name):
    ...
    if response.status_code == 200:
        ...
        return next((checklist['id'] for checklist in checklists if checklist['name'] == checklist_name), None)
    else:
        print(f"Erro ao buscar checklists: {response.text}")
        return None

# Função para criar uma checklist
def create_checklist(card_id, checklist_name):
    existing_checklist_id = checklist_exists(card_id, checklist_name)
    if existing_checklist_id:
        print(f"Checklist '{checklist_name}' já existe.")
        return existing_checklist_id
    ...
    response = requests.post(url, params=query)
    if response.status_code == 200:
        print(f"Checklist '{checklist_name}' criada com sucesso.")
        return response.json()['id']
    else:
        print(f"Erro ao criar checklist '{checklist_name}': {response.text}")
        return None

# Função para verificar se um item já existe em uma checklist
def item_exists_in_checklist(checklist_id, item_name):
    ...
    if response.status_code == 200:
        ...
        print(f"Erro ao buscar itens da checklist: {response.text}")
        return False

# Função para adicionar itens na checklist
def add_item_to_checklist(checklist_id, item_name):
    if item_exists_in_checklist(checklist_id, item_name):
        print(f"Item '{item_name}' já existe na checklist.")
        return
