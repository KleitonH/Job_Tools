    ...
    if option == "1": # Se a opção for 1, inicia a operação
        ...
        while(time.time() - tempo_inicial) < tempo_de_execucao and Breaker.verificar_interrupcao(): # Loop de execução enquanto o contador de tempo estiver ativo
            ...
            if watch in textwatcher or watch2 in textwatcher: # Se a palavra-chave 1 ou 2 estiverem no texto
                ...
                time.sleep(0.9) 
                pyautogui.click(124, 667) # Clica na outra aba de sistema aberta
                time.sleep(1)

                screenshotwatcher2 = pyautogui.screenshot(region=(77, 640, 280, 25)) # Segundo método de verificação, segue a mesma lógica do primeiro, buscando outras palavras. Garante que esteja na tela correta de operação
                ...
                if watch3 in textwatcher2: # Se a palavra-chave estiver no texto
                    ...
                    if errorname in texterror: # Se a palavra-chave estiver no texto
                        ...
                        if any(numero >= 1 for numero in numeros_textdate) or any(numero >= 1 for numero in numeros_textdate2): # Se o número for maior ou igual a 1, houve movimentação, não sendo necessária a inativação
                            ...
                            time.sleep(0.5) # Intervalo de segurança
                            screenshoterror2 = pyautogui.screenshot(region=(330, 305, 692, 141)) # Captura a mensagem de erro de inativação, neste caso, se o item já estiver inativo ou se há duplicação de código de barra, o que bloqueia inativação
                ...
                pyautogui.click(222, 753) # Clica na posição do ícone da barra de tarefas do sistema
                time.sleep(0.5) # Intervalo de segurança
                pyautogui.click(337, 667) # Clica na outra aba de sistema aberta da lista de item
