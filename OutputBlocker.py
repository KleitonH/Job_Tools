...
try: # Tenta abrir o arquivo de lista de MECANICA
    ...
    lista_mecanica = ["ACOPLAMENTO BARRA CAMB", "ADAPTADOR TAMPA TANQ", "BARRA AXIAL", "BOIA COMB", "DISCO FREIO", "","HELICE", "JOGO JUNTA", "JUNTA ANEL", "JUNTA CABECOTE", "JUNTA CAMBIO", "JUNTA COLETOR", "JUNTA MOTOR", "JUNTA RETIFICA", "PIVO SUSPENSAO", "SUPORTE BRACO TENSOR", "TERMINAL CAIXA CAMB", "TERMINAL DIRECAO", "TUBO ABASTECIMENTO"]
    ...
    if option == "1": # Se a opção for 1, inicia a operação
        ...
        while(time.time() - tempo_inicial) < tempo_de_execucao and Breaker.verificar_interrupcao(): # Loop de execução enquanto o contador de tempo estiver ativo
            ...
            if watch in textwatcher or watch2 in textwatcher: # Se a palavra-chave 1 ou 2 estiverem no texto
                ...
                screenshotcod = pyautogui.screenshot(region=(37, 743, 59, 15)) # Variável para receber código da célula do item da tabela
                ...
                textwatcher2 = pytesseract.image_to_string(screenshotwatcher2, lang='por') # Converte para texto
                watch3 = "sistan" # Palavra-chave
                time.sleep(0.5) # Intervalo de segurança
                print(screenshotwatcher2)
                ...
                time.sleep(0.4) # Intervalo de segurança 
                pyautogui.press("f3") # Abre a aba de modificação de itens
                time.sleep(0.6) # Intervalo de segurança
                pyautogui.typewrite(textcod) # Digita o código do item
                time.sleep(0.9) # Intervalo de segurança
                pyautogui.press("enter") #Seleciona o item
                time.sleep(0.9) # Intervalo de segurança
                pyautogui.press("enter") #Seleciona dentre similares
                time.sleep(1.2) # Intervalo de segurança
                pyautogui.press("enter") # Caso estiver inativado, fecha a mensagem de erro
                time.sleep(0.9) # Intervalo de segurança
                pyautogui.click(574, 529) # Clica no botão de fechar o erro
                time.sleep(0.1) # Intervalo de segurança
                pyautogui.click(574, 529) # Clica no botão de fechar o erro,
                pyautogui.typewrite("2") # Digita 2 para limitar a saída de 2 itens juntos, neste caso, para correção de limite de venda de discos de freio
                time.sleep(0.9) # Intervalo de segurança
                pyautogui.click(656, 659) # Clica no botão de fechar o erro
                time.sleep(0.9) # Intervalo de segurança
                pyautogui.press("esc") # Fecha a aba de modificação
                time.sleep(0.9) # Intervalo de segurança
